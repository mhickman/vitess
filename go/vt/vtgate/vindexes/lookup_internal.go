...
func (lkp *lookupInternal) Init(lookupQueryParams map[string]string, autocommit, upsert bool) error {
	...
	lkp.sel = fmt.Sprintf("select distinct %s, %s from %s where %s in ::%s", lkp.FromColumns[0], lkp.To, lkp.Table, lkp.FromColumns[0], lkp.FromColumns[0])
	lkp.ver = fmt.Sprintf("select distinct %s from %s where %s = :%s and %s = :%s", lkp.FromColumns[0], lkp.Table, lkp.FromColumns[0], lkp.FromColumns[0], lkp.To, lkp.To)
